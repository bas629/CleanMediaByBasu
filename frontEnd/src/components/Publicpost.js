...
function Publicpost() {
  const { userId, savePost, setSavePost, ProtectedFriend, setProtectedFriend, getPublicPost, pPost, flag, AddFriend,Unfriend, Name, setFlag } = useContext(ContextApp);
  const [validImages, setValidImages] = useState({});

  useEffect(() => {
    getPublicPost();
    console.log(pPost);
    const interval = setInterval(getPublicPost, 2000); // every 2 seconds
      ...
      <div className="flex flex-col min-h-screen bg-gray-900">
        ...
        <div className="sticky top-0 bg-gray-900 border-b border-gray-700 shadow-sm z-40 p-4">
          <div className="flex justify-between items-center max-w-4xl mx-auto">
            <h1 className="text-2xl font-bold text-white">Public Page</h1>
            <button
                ...
                <div key={p._id} className="bg-gray-800 rounded-xl shadow-lg overflow-hidden transition-all duration-200 hover:shadow-xl border border-gray-700">
                  ...
                  <div className="p-4 flex justify-between items-center">
                    ...
                    {ProtectedFriend?.includes(p._id) ? (
                      <button className="px-4 py-2 bg-green-500 text-white rounded-full text-sm font-medium hover:bg-green-600 transition-colors duration-200" onClick={()=>Unfriend(p._id)}>
                        Unfriend
                      </button>
                    ) : (
                      <button
