...
function PrivatePost() {
  ...
  const { savePost, changeHandler, PrivateFriend, AddFriend, ProtectedFriend, userId, getPublicPost, privatePost,Unfriend, PrivatePost } = useContext(ContextApp);
              ...
              <div key={p._id} className="bg-gray-800 rounded-xl shadow-lg overflow-hidden transition-all duration-200 hover:shadow-xl border border-gray-700">
                ...
                <div className="p-4 flex justify-between items-center">
                  ...
                  {ProtectedFriend?.includes(p._id) ? (
                    <button   onClick={()=>Unfriend(p._id)} className="px-4 py-2 bg-green-500 text-white rounded-full text-sm font-medium hover:bg-green-600 transition-colors duration-200">
                      UnFriends
                    </button>
                  ) : (
                    <button
